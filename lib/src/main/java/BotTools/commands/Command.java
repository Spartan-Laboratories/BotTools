...
public abstract class Command{
	...
	protected ReplyCallbackAction reply;
	 ...
	 */
	public Command setEvent(MessageReceivedEvent event) {
		messageEvent = event;
		...
		tagged = message.getMentions().getMembers();
	...
	protected void show(File file) {
		BotAction.sendFileInChannel(getChannel(), file);
	}
	...
	protected void generateSlashCommandData() {
		slashCommandData = Commands.slash(commandName, getDescription());
	...
	protected MessageChannel setChannel(MessageChannel messageChannel) {
		return channel = messageChannel;
	}
	protected MessageChannel resetChannel() {
		return channel = (TextChannel)(
				messageEvent != null ? messageEvent.getChannel() : scEvent.getMessageChannel());
	}
