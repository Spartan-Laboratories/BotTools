...
public class GuildDataParser {
	...
	private void updateServerDatabase(Guild guild) {	
		...
		for(Member m: actualMembers){						
			...
			if(!isInSystem){
				...
				reader.newChild(personTag, "keyid");
	...
	public void addReactionRole(Guild guild, Emoji emote, Role role) {
		setDocument(guild, "GuildData.xml");
		Node rrsNode = reader.getChild(reader.getRoot(), "reactionRoles"),
			 rrNode = reader.newChild(rrsNode, "reactionRole");
		reader.newChild(rrNode, "emote", emote.getName());
	...
	public String getCorrespondingRoleID(Guild guild, Emoji emote) {
		setDocument(guild, "GuildData.xml");
		Node rrsNode = reader.getChild(reader.getRoot(), "reactionRoles");
		for(Node n: reader.getChildren(rrsNode))
			if(reader.getValue(reader.getChild(n, "emote")).equals(emote.getName()))
				return reader.getValue(reader.getChild(n, "role"));
		return null;
	}
	
	protected void setDocument(Guild guild, String docName) {
		setGuildString(guild);
	...
	public void modGuildDataValue(Guild guild, String name, String value) {
		setDocument(guild, "GuildData.xml");
		modCurrentDocValue(name, value);
	}
	public String getGuildDataValue(Guild guild, String name) {
		setDocument(guild, "GuildData.xml");
		return getCurrentDocValue(name);
	}
	public String getUserValue(Guild guild, Member member, String name) {
		Node person = goToPerson(guild, member);
		return reader.getValue(reader.getChild(person, name));
	}
	public void setUserValue(Guild guild, Member member, String name, String value) {
		Node person = goToPerson(guild, member);
		reader.setValue(reader.getChild(person, name), value);
	}
	private Node goToPerson(Guild guild, Member member) {
		setDocument(guild, "MemberData.xml");
		return getPersonNode(member.getUser().getName());
	}
	private void modCurrentDocValue(String name, String value) {
